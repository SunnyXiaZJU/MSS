...
while ~converge
    ...
    C2_new=soft_thresh_original(J_new+Lambda2/mu2,lambda2/mu2);
    C2_new(diag_idx)=0;
    C1_new=sigma_soft_thresh_original(J_new+Lambda3/mu3,lambda1/mu3);
    
     if sum(isnan(C1_new(:))) || sum(isinf(C1(:)))
        pause;
    end
