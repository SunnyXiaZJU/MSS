...
%cd '../data/hopkins/Hopkins155/';
%cd '/media/xyq/tools/aliyun/firstProject/data/Hopkins155'
cd '/media/xyq/work/xyq/firstProject/data/Hopkins155'
addpath(genpath( '/media/xyq/work/xyq/firstProject/code/3rdparty'));
%addpath '/media/xyq/tools/aliyun/firstProject/code/3rdparty/SSC_ADMM_v1.1';
%addpath(genpath( '/media/xyq/tools/aliyun/firstProject/code/3rdparty'));
...
for i = 1:length(d)
    if ( (d(i).isdir == 1) && ~strcmp(d(i).name,'.') && ~strcmp(d(i).name,'..') )
        ...
        if (foundValidData)
            ...
            [missrate1,C1] = SSC(X,s,r,affine,alpha,outlier,rho);
            
            r = 4*n; affine = true; outlier = false; rho = 0.7;
            [missrate2,C2] = SSC(X,s,r,affine,alpha,outlier,rho);
